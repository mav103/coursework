class WardsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ward.save
        format.html { redirect_to @ward, notice: 'Палата создана.' }
  ...
  def update
    respond_to do |format|
      if @ward.update(ward_params)
        format.html { redirect_to @ward, notice: 'Палата изменена.' }
  ...
  def destroy
    @ward.destroy
    respond_to do |format|
      format.html { redirect_to surveys_url, notice: 'Палата удалена.' }
  ...
  private
    ...
    def ward_params
      params.require(:ward).permit(:department, :floor, :capacity, :number, :num_of_pacients)
    end
end
